...
// goroutines, etc.)
const ATOMIC_MODIFICATION_LOCK_QUEUE_SZ = 4

// active modification lock sleeps for 10 us each check
const ATOMIC_MODIFICATION_LOCK_SLEEP = 10 * time.Microsecond

type ActiveModificationLock struct {
	arr          []int
	ticket       uint32
	dequeueCount uint32
	nReaders     uint32
...
func NewActiveModificationLock() *ActiveModificationLock {
	abql := ActiveModificationLock{
...
func (l *ActiveModificationLock) RLock() {
	ticket := atomic.AddUint32(&l.ticket, 1) - 1
	for ticket-l.dequeueCount >= uint32(ATOMIC_MODIFICATION_LOCK_QUEUE_SZ) {
		time.Sleep(ATOMIC_MODIFICATION_LOCK_SLEEP)
	}
	for l.arr[ticket%uint32(ATOMIC_MODIFICATION_LOCK_QUEUE_SZ)] != 1 {
		time.Sleep(ATOMIC_MODIFICATION_LOCK_SLEEP)
	}
	// increment nReaders
	...
	l.arr[int(ticket)%ATOMIC_MODIFICATION_LOCK_QUEUE_SZ] = 0
	l.arr[int(ticket+1)%ATOMIC_MODIFICATION_LOCK_QUEUE_SZ] = 1
	atomic.AddUint32(&l.dequeueCount, 1)
	return
}

func (l *ActiveModificationLock) Lock() uint32 {
	ticket := atomic.AddUint32(&l.ticket, 1) - 1
	for ticket-l.dequeueCount >= uint32(ATOMIC_MODIFICATION_LOCK_QUEUE_SZ) {
		time.Sleep(ATOMIC_MODIFICATION_LOCK_SLEEP)
	}
...
func (l *ActiveModificationLock) RUnlock() {
	// decrement nReaders
	atomic.AddUint32(&l.nReaders, ^uint32(0))
}

func (l *ActiveModificationLock) Unlock(ticket uint32) {
	l.arr[int(ticket)%ATOMIC_MODIFICATION_LOCK_QUEUE_SZ] = 0
	l.arr[int(ticket+1)%ATOMIC_MODIFICATION_LOCK_QUEUE_SZ] = 1
	atomic.AddUint32(&l.dequeueCount, 1)
}
