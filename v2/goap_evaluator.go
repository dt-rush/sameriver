...
func (e *GOAPEvaluator) applyPath(path []*GOAPAction, ws *GOAPWorldState) (result *GOAPWorldState, ok bool) {
	result = ws.copyOf()
	for i, action := range path {
		if i > 0 && !e.presFulfilled(action, result) {
			return nil, false
		}
...
func (e *GOAPEvaluator) prepend(
	...
	newResult, ok := e.applyPath(newPath, nothing)
	if !ok {
		debugGOAPPrintf("    --- leads to invalid pre for some path action")
		return nil, false
	}
	...
	if closerToGoal || betterThanNothing || helpsWithAPre {
		...
		// add heuristic
		cost += nUnfulfilledMain + nUnfulfilledPres + len(newPath)
